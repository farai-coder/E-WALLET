...
const Entrances = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/analytic/all_vehicle_history/?mode=Visitor`);
        ...
        if (Array.isArray(response.data)) {
          ...
          // Fetch vehicle history for each entrance by number_plate
          const updatedEntrances = await Promise.all(response.data.map(async (entrance) => {
            try {
              const historyResponse = await axios.get(`${BASE_URL}/history/${entrance.number_plate}`);
              return { ...entrance, entranceHistory: historyResponse.data }; // Add entranceHistory to the vehicle
            } catch (error) {
              console.error(`Error fetching history for ${entrance.number_plate}:`, error);
              return entrance; // In case of error, just return the entrance without history
            }
          }));

          setEntrancesData(updatedEntrances); // Update the state with the entrance history data
        } else {
  ...
  const handleOpenModal = (entrance) => {
    setSelectedVehicle(entrance);
    setOpenModal(true);
  };

...
function ImportVisitor(props) {
  ...
  console.log('scv', file)
  const handleImportClose = () => {
    // Code to close the modal
    document.getElementById("import_visitor_modal").style.display = "none";
    props.setShowImportVisitor(false); // Call setShowImportVisitor here
  };
  ...
  const handleUploadClick = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/upload_visitor_file/`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="container" id='import_visitor_modal' style={{ 
      position: 'absolute', 
      top: 0, 
      right: 0, 
      left: 0, 

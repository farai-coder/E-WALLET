...
function Connectivity() {
  ...
  const handleAddSave = async () => {
    try {
      // Send the data to the API
      await axios.post(`${BASE_URL}http://localhost:8000/devices/`, newDevice, {
          ...
          {server && (
            <>
              ...
              <TextField
                ...
                onChange={(e) => setServer({ ...server, ip_address: e.target.value })}
              />
              <TextField
                label="Subnet Mask"
                fullWidth
                margin="normal"
                value={server.subnet_mask}
                onChange={(e) => setServer({ ...server, subnet_mask: e.target.value })}
              />
              <TextField
                label="Port Number"
                fullWidth
